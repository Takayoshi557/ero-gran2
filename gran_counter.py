...
#            worksheet_list.update_cell(id_cell.row, 11, str(rbun_buyer))   # 購入者入力
            ...
            dia = int(rbun_dia)
            bun_buyer = worksheet_list.cell(id_cell.row, 12).value
            id_check = list()
            cama_list = list()
            death_list = list()
            samurai_list = list()
            ...
            if pp < 10 and dia < 5000:
                ...
                if 10 <= pp < 25 and dia >= 5000:
                    ...
                    if tema < 500:
                        bunpb = (dia - ketsu * 3 - tema) / pp
                        await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) +' が' + str(dia) + ' diaで売れました。<@' + str(bun_buyer) + '>と取引を行って下さい。\n血盟資金:' + str(math.floor(ketsu)) + 'diaを各盟主へ渡してください。\nメンションされている方々は ' + str(math.floor(bunpb)) + 'diaで出品して下さい。\nちなみに手間賃' + str(math.floor(tema)) + 'diaです。\n血盟資金受取\n<@363032621845839892>\n<@477504935727071232>\n<@290377448711782400>\n\n分配\n' + str(id_check))
                    elif tema >= 500:
                        tema = 500
                        bunpb = (dia - ketsu * 3 - tema) / pp
                        await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) +' が' + str(dia) + ' diaで売れました。<@' + str(bun_buyer) + '>と取引を行って下さい。\n血盟資金:' + str( math.floor(ketsu)) + 'diaを各盟主へ渡してください。\nメンションされている方々は ' + str(math.floor(bunpb)) + 'diaで出品して下さい。\nちなみに手間賃は上限の' + str(math.floor(tema)) + 'diaです。\n血盟資金受取\n<@363032621845839892>\n<@477504935727071232>\n<@290377448711782400>\n\n分配\n' + str(id_check))
                    ...
                    if bunpb < 50:
                        ...
                        await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) +' が' + str(dia) + ' diaで売れました。\n10人以上, 5000dia未満なので以下となります。\nメンションされている方々は ' + str(math.floor(bunpb)) + 'diaで出品して<@' + str(bun_buyer) + '>と取引して下さい。\n分配者手数料は' + str(math.floor(tema)) + 'diaです。\n血盟資金はありません。\n' + str(id_check))
                        return
                else:
                    if pp >= 25 and dia >= 5000:
                        ketsushi = dia * 0.03
                        ...
                        if bunpc < 100:
                            ...
                            await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) +' が' + str(dia) + ' diaで売れたので分配を行います。\n25人以上 / 分配 100dia未満なので全額血盟資金となります。\n<@' + str(bun_buyer) + '>は' + str(
                                math.floor(meishubun1)) + 'diaを以下の各盟主に渡してください。\n<@363032621845839892>\n<@477504935727071232>\n<@290377448711782400>')
                            return
                        else:
                            cama_num = 0
                            death_num = 0
                            ...
                            await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) + ' が' + str(dia) + ' diaで売れました。\n25人以上 / 分配 100dia以上なので盟主が分配します。<@' + str(bun_buyer) + '>さんは以下に従って盟主と取引して下さい。\n尚、血盟資金 ' + str(math.floor(ketsushi)) + 'diaも含まれています。')
                            await culc_channel.send('<@477504935727071232>さん： ' + str(math.floor(bun_cama)) + ' diaを受取り、以下の方に ' + str(math.floor(bunpc)) + ' diaを分配下さい。\n' + str(cama_list))
                            await culc_channel.send('<@363032621845839892>さん： ' + str(math.floor(bun_samurai)) + ' diaを受取り、以下の方に ' + str(math.floor(bunpc)) + ' diaを分配下さい。\n' + str(samurai_list))
                            await culc_channel.send('<@290377448711782400>さん： ' + str(math.floor(bun_death)) + ' diaを受取り、以下の方に ' + str(math.floor(bunpc)) + ' diaを分配下さい。\n ' + str(death_list))
                            return
                        ...
                        if bunpd < 100:
                            meishubun2 = dia / 3
                            await culc_channel.send(str(rbun_id) + 'の' + str(worksheet_list.cell(id_cell.row, 2).value) + '/' + str(worksheet_list.cell(id_cell.row, 3).value) +' が' + str(dia) + ' diaで売れたので分配を行います。\n25人以上で分配が100dia/人 未満なので全額血盟資金となります。\n<@' + str(bun_buyer) + '>さんは'+ str(math.floor(meishubun2)) + 'diaを以下の各盟主に渡してください。\n<@363032621845839892>\n<@477504935727071232>\n<@290377448711782400>')
                        else:
                            cama_num = 0
                            death_num = 0
                            samurai_num = 0
                            ...
                            await culc_channel.send('<@477504935727071232>さんに' + str(cama_bun_total) + ' dia を渡してください。')
                            await culc_channel.send('<@363032621845839892>さんに' + str(samurai_bun_total) + ' dia を渡してください。')
                            await culc_channel.send('<@290377448711782400>さんに' + str(death_bun_total) + ' dia を渡してください。')
                            ...
                            if cama_num >= 10:
                                ...
                                await culc_channel.send('<@477504935727071232>さんは以下の方々に' + str(
                                    cama_bun) + ' dia を渡してください。\nまた10名以上なので血盟資金が' + str(
                                    cama_ketsu) + 'dia 発生していますので受領下さい。\n' + str(cama_list))
                                ...
                                if death_num >= 10:
                                    ...
                                    await culc_channel.send('<@290377448711782400>さんは以下の方々に' + str(
                                        death_bun) + ' dia を渡してください。\nまた10名以上なので血盟資金が' + str(
                                        death_ketsu) + 'dia 発生していますので受領下さい。\n' + str(death_list))
                                    ...
                                    if samurai_num >= 10:
                                        ...
                                        await culc_channel.send('<@363032621845839892>さんは以下の方々に' + str(
                                            samurai_bun) + ' dia を渡してください。\nまた10名以上なので血盟資金が' + str(
                                            samurai_ketsu) + 'dia 発生していますので受領下さい。\n' + str(samurai_list))
                                    else:
                                        samurai_bun_total = bunpd * samurai_num
                                ...
                                await culc_channel.send(
                                    '<@477504935727071232>さんに' + str(cama_bun_total) + ' dia を渡してください。')
                                await culc_channel.send('<@477504935727071232>さんは以下の方々に' + str(
                                    cama_bun) + ' dia を渡してください。\n' + str(cama_list))
                                if death_num >= 10:
                                    death_ketsu = death_bun_total * 0.03
